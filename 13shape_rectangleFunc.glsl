...
#define TWO_PI 6.28318530718
 ...
 //    return vec3(abs(q.z + (q.w - q.y) / (6.0 * d + e)), d / (q.x + e), q.x);
	...
	void main() {
		...
		float rectOne = drawRectangle(st, 0.7, vec2(0.1, 0.1), 0.002);
		float rectTwo = drawRectangle(st, 0.5, vec2(0.1, 0.5), 0.002);

		color = mix(color, purple, rectOne);
		color = mix(color, blue, rectTwo);
