...
#define QTR_PI .78539816339
	...
	void main() {
		...
		colour+=cutout(stroke(raysSDF(st,28),.5,.2), fill(st.y,.5));
		// colour+=stroke(raysSDF(st,18),.5,.35);
		float inner=starSDF(st.xy,4,.3);
		float outer=starSDF(st.yx,2,.19);
		colour*=step(.7,outer);
		colour+=fill(outer,.5);
		colour-=stroke(inner,.25,.06);
		colour+=stroke(outer,.6,.05);
