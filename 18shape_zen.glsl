...
#define TWO_PI 6.28318530718
 ...
 //    return vec3(abs(q.z + (q.w - q.y) / (6.0 * d + e)), d / (q.x + e), q.x);
	...
	void main() {
		...
		vec3 color = vec3(fract(d*(u_mouse.x/200.0)));
		color *= purple;


		// pct = 1.0-smoothstep(0.3, 0.31, distance(st, vec2(0.3, 0.4)));
		float circleOne = drawCircle(st, 30.0+(5.0*(sin(u_time))), 0.01, vec2(0.3, 0.4));
		float circleTwo = drawCircle(st, 20.0+(10.0*(sin(u_time+1.0))), 0.01, vec2(0.6, 0.7));

		// color = mix(color, purple, st.x);
