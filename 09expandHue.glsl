...
#define TWO_PI 6.28318530718
 ...
 //    return vec3(abs(q.z + (q.w - q.y) / (6.0 * d + e)), d / (q.x + e), q.x);
    ...
    float angle = parabola(0.1,atan(toCenter.y,toCenter.x));
    // float angle = impulse(toCenter.y,toCenter.x);
    float radius = length(toCenter)*1.6;

    // float hue = ((angle/TWO_PI)+0.5)*abs(sin(u_time));
    float hue = ((angle/TWO_PI)+0.5);
    // float hue = parabola(radius, angle);
