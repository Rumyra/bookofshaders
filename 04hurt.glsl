...
#define PI 3.14159265359
	...
	// The task for this is to create a colour mix based on an emotion - I recently felt hurt by a personal event, so I'm going to use that as inspiration
	vec3 confusion = vec3(0.400, 0.300, 0.100);
	vec3 pain = vec3(0.050, 0.100, 0.100);
	vec3 frustration = vec3(0.800, 0.700, 0.000);
	...
	void main() {
		...
		float mixValOne = abs(expStep(u_time, 2.0, 1.0));
		float mixValTwo = abs(gain(u_time, 5.0));

		color = mix(confusion, mix(pain, frustration, mixValTwo), mixValOne);
